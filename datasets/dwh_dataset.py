...
CLASSES = ('Water', 'oil', 'Boundary')
PALETTE = [[255, 255, 0], [0, 0, 255], [0, 0, 0]]

ORIGIN_IMG_SIZE = (128, 128)
INPUT_IMG_SIZE = (128, 128)
# INPUT_IMG_SIZE = (256, 256)
TEST_IMG_SIZE = (128, 128)


get_training_transform = transforms.Compose([
...
#     crop_aug = Compose([RandomScale(scale_list=[0.5, 0.75, 1.0, 1.25, 1.5], \
#                                     mode='value'),
#                         SmartCropV1(crop_size=128,  \
#                                     max_ratio=0.75, \
...
#     # print("img", type(img), "mask", type(mask))
#     return img, mask
...
get_val_transform = transforms.Compose([
...
def val_aug(img, mask):
    ...
    # mask = get_val_transform(mask.copy())
    # print("### img ###", img.shape, "### mask ###", mask.shape)
    # img, mask = aug['image'], aug['mask']
    # print("img", img.shape, "mask", mask.shape)
    # print("img", type(img), "mask", type(mask))
...
class DWHDataset(Dataset):
    def __init__(self, data_root='data/vaihingen/test', mode='val', 
                 img_dir='images_128', mask_dir='masks_128', 
                 img_suffix='.mat', mask_suffix='.mat', 
    ...
    def __getitem__(self, index):
        ...
        #     # (3, 128, 128) (128, 128)
        #     # print("val img", img.shape, "val mask", mask.shape)
        #     # print("img", type(img), "mask", type(mask))  # numpy.ndarray
        #     # if self.transform:
        ...
        #     img, mask = self.load_img_and_mask(index)
        #     # print("train img", img.shape, "train mask", mask.shape)
        #     # if self.transform:
        #     #     img, mask = self.transform(img, mask)
        img, mask = self.load_img_and_mask(index)
    ...
    def load_img_and_mask(self, index):
        ...
        img = loadmat(img_name)['img'].transpose(2, 0, 1)
        mask = loadmat(mask_name)['map'][:, :, 0]
        # img (3, 128, 128) mask2 (128, 128)
        # print("img", img.shape, "mask", mask.shape) 
    ...
    def load_mosaic_img_and_mask(self, index):
        ...
        # print("img_a", img_a.shape, "mask_a", mask_a.shape)
        ...
        print("trian img", img.shape, "trian mask", mask.shape)
