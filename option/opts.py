...
def get_options():
	...
	parser.add_argument('--data_name', type=str, choices=['OSTD_SAR', 'OSTD_Image', 'vaihingen', "potsdam"],
						 default='vaihingen', help='Dataset the model is applied to and trained on')
	...
	parser.add_argument('--patch_size', type=int, default=256, help='size of the image patches the model should be trained on')
	...
	parser.add_argument('--model', choices=['unet', 'segformer', 'segformer-b5', 'AMSUnet', "MANet",
						...
						default='AMSUnet', help="the model architecture that should be trained")
	parser.add_argument('--loss_function', type=str, choices=["abcLoss", "UnetFormerLoss", 'dice', 'jaccard', 'focal', 'cross-entropy', 'weighted-CE'],
						default='abcLoss', help='loss function that should be used for training the model')
